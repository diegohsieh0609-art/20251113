...
class SideMenu {
    ...
    checkHover() {
        ...
        // 只有當滑鼠既不在主選單項目上，也不在子選單上時，才隱藏子選單
        const mainItemHasSubmenu = this.hoveredItemIndex !== -1 && this.items[this.hoveredItemIndex].submenu;
        // ***** 修改：使用計時器延遲隱藏 *****
        if (!mainItemHasSubmenu && !this.isMouseOverSubmenu() && this.submenuTargetFade > 0) {
